...
const chOverrides: Record<string, Record<string, unknown>> = {
  en: enOverrides,
  ja: jaOverrides,
  ko: koOverrides,
  de: deOverrides,
...
/**
 * Get merged translations (shared + CH overrides) for a given locale.
...
export function getTranslationsForLocale(locale: string) {
  ...
  const overrides = chOverrides[validLocale] || {};
