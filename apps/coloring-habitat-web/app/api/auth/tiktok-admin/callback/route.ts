...
export const GET = async (request: NextRequest) => {
  ...
  if (!code) {
    return NextResponse.redirect(
      new URL(
        "/admin/social?error=No authorization code received",
        request.url,
      ),
    );
  }
  ...
  if (!storedState || storedState !== state) {
    return NextResponse.redirect(
      new URL("/admin/social?error=Invalid state parameter", request.url),
    );
  }
  ...
  if (!clientKey || !clientSecret) {
    return NextResponse.redirect(
      new URL(
        "/admin/social?error=TikTok credentials not configured",
        request.url,
      ),
    );
  }
  ...
  const redirectUri = `${baseUrl}/api/auth/tiktok-admin/callback`;
  ...
  try {
    ...
    const response = NextResponse.redirect(
      new URL("/admin/social?success=tiktok", request.url),
    );
