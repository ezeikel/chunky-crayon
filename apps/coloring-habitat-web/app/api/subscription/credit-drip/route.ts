...
import { PLAN_CREDITS_MONTHLY, PLAN_ROLLOVER_CAPS } from "@/utils/stripe";
...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    // Find all active annual subscriptions for Coloring Habitat
    const activeAnnualSubscriptions = await db.subscription.findMany({
      where: {
        status: SubscriptionStatus.ACTIVE,
        billingPeriod: BillingPeriod.ANNUAL,
        user: { brand: "COLORING_HABITAT" },
      },
      include: {
        user: true,
      },
    ...
    for (const subscription of activeAnnualSubscriptions) {
      try {
        ...
        const rolloverCap = PLAN_ROLLOVER_CAPS[planName] ?? 0;

        if (!monthlyCredits) {
          console.log(`Skipping ${user.email} - unknown plan ${planName}`);
          results.skipped++;
          continue;
        }
    ...
    return NextResponse.json({
      status: "success",
      service: "coloring-habitat-credit-drip",
      month: monthYear,
      summary: {
