...
const OnboardingCarousel = ({ onComplete }: OnboardingCarouselProps) => {
  ...
  const isPaywallSlide = activeIndex === SLIDE_COUNT - 1;

  return (
    <LinearGradient colors={["#FDFAF5", "#F5EEE5"]} style={styles.container}>
      {/* Skip button — hidden on paywall slide */}
      {!isPaywallSlide && (
        <Pressable
      ...
      >
        ...
        {/* Slide 4: Paywall */}
        <OnboardingPaywallSlide onComplete={onComplete} />
      </ScrollView>

      {/* Bottom area: dots + next button (hidden on paywall) */}
      {!isPaywallSlide && (
        <View
