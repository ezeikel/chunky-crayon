...
export const GET = async (request: Request) => {
  ...
  try {
    ...
    // manual TikTok posting before auto-posts fire. Mon-Sat the
    // strip's already out; including it would just be noise.
    //
    ...
    // you can post the carousel via the TikTok app.
    ...
    // content reel AND no comic strip AND no blog post.
    const hasAnyContent =
      coloringImage || demoReelImage || contentReel || comicStrip || blogPost;
    if (!hasAnyContent) {
      const message =
      ...
      await sendAdminAlert({
        ...
        body: `The social digest cron was skipped because no daily image, demo reel, content reel, comic strip, or blog post was ready for today.\n\nCheck the worker logs and Vercel function logs.`,
      });
