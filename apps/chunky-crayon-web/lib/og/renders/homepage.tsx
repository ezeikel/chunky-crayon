...
/**
 * Build an ImageResponse for the homepage OG. Used by both the dynamic
 ...
 * dynamic render).
 */
export const renderHomepageOGImageResponse = async (
  tagline: string,
  ...
  const fontExports = buildFontExports(fonts);

  if (featured.length === 0) {
    return new ImageResponse(renderTextFallback(tagline, logo), {
      width: OG_WIDTH,
      height: OG_HEIGHT,
      fonts: fontExports,
    });
  ...
  return new ImageResponse(renderCollage(featured, tagline, logo), {
    width: OG_WIDTH,
    height: OG_HEIGHT,
    fonts: fontExports,
  });
