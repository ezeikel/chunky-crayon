 ...
 *   pnpm tsx apps/chunky-crayon-web/scripts/migrate-blob-urls-to-r2.ts --dry-run
 *
 *   # Apply changes
 *   pnpm tsx apps/chunky-crayon-web/scripts/migrate-blob-urls-to-r2.ts
 *
 * Run with dotenv preload:
 *   DOTENV_CONFIG_PATH=apps/chunky-crayon-web/.env.local pnpm tsx -r dotenv/config apps/chunky-crayon-web/scripts/migrate-blob-urls-to-r2.ts
 */
  ...
  for (const image of images) {
    ...
    if (Object.keys(updates).length > 0) {
      if (isDryRun) {
        ...
        console.log(
          `✅ Updated image ${image.id} (${Object.keys(updates).length} fields)`,
        );
      }
      updated++;
    } else {
...
async function main() {
  ...
  console.log(
    `Coloring Images: ${coloringResults.updated} updated, ${coloringResults.skipped} skipped`,
  );
  console.log(
    `Saved Artwork: ${artworkResults.updated} updated, ${artworkResults.skipped} skipped`,
  );
  console.log('─'.repeat(40));
  console.log(
    `Total Updated: ${coloringResults.updated + artworkResults.updated}`,
  );
