...
/**
 * Mirror of apps/chunky-crayon-web/lib/colo/service.test.ts. Same
 * pure-function contract on both sides — same fixtures, same expected
 * shapes. If the COLO_STAGES thresholds in catalog.ts ever drift
 * between web and mobile, these tests catch it.
 */
describe("getColoState — progressToNext", () => {
  ...
  it("caps current at required when the kid overshoots the next-stage threshold", () => {
    ...
    // consumers showed "50/30 artworks" in the tooltip AND
    // "Save -20 more artworks to evolve!" in the DashboardHeader
    // (which does required - current). The fix clamps current at
    // required so display math stays sane.
