...
// option is INERT on the SDK 56 precompiled-modules Podfile template (it records
// apple.extraPods in Podfile.properties.json but the template never reads it), so
// we patch the generated Podfile directly. ios/ is regenerated by prebuild, so
// this is a config plugin, not a hand-edit. Inserted right after
// use_expo_modules! inside the app target.
...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return withModularHeaders(
    withRemoveActivityRecognition(withGradleHeap(expoConfig)),
  ) as ExpoConfig;
};
