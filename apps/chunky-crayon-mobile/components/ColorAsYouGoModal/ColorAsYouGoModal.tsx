...
const CREDIT_AMOUNT_FALLBACK: Record<string, number> = {
  cayg_credits_50_v1: 50,
  cayg_credits_200_v1: 200,
  cayg_credits_500_v1: 500,
};

// Hook to fetch credit packs offering
function useColorAsYouGoPacksOffering() {
  return useQuery({
    queryKey: ["revenuecat", "colorAsYouGoPacks"],
    queryFn: async () => {
      const offerings = await Purchases.getOfferings();
      return offerings.all["color_as_you_go"] ?? null;
    },
...
function usePurchaseColorAsYouGoPack() {
...
const ColorAsYouGoModal = ({
  visible,
  onClose,
  onSuccess,
  skipParentalGate = false,
}: ColorAsYouGoModalProps) => {
  const insets = useSafeAreaInsets();
  const { data: offering, isLoading } = useColorAsYouGoPacksOffering();
  const purchaseMutation = usePurchaseColorAsYouGoPack();
    ...
    <>
      ...
      >
        <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          >
            {/* Hero — same fanned coloring pages as the subscription
                paywall, so the credit-pack surface feels like one family
                and leads with what the credits MAKE. */}
            <PaywallHero play={visible} />

            <Text style={styles.title}>Color as you go</Text>
              ...
              <View style={styles.packs}>
                {packages.map((pkg) => (
                  <CreditPackRow
                    ...
                    isBestValue={pkg.identifier === "cayg_credits_200"}
...
export default ColorAsYouGoModal;
