/**
 * Shared prompts + palette for the coloring-image post-processing pipeline.
 *
 * Imported by record/region-store.ts and record/fill-points.ts. Both call
 * the same coloring-core entry points (generateRegionStoreLogic,
 * generateRegionFillPointsLogic) which take the same `regionStoreConfig`
 * shape — keeping the prompts in one place lets us update the wording
 * once instead of in two parallel files.
 *
 * Ported verbatim from apps/chunky-crayon-web/lib/ai/prompts.ts +
 * apps/chunky-crayon-web/constants.ts. Keep in sync with the web copy.
 */
import { DEFAULT_PALETTE_VARIANT_MODIFIERS } from "@one-colored-pixel/coloring-core";
...
// Ported from apps/chunky-crayon-web/constants.ts (ALL_COLORING_COLORS_EXTENDED).
...
/**
 * The shape both `generateRegionStoreLogic` and
 * `generateRegionFillPointsLogic` (from `@one-colored-pixel/coloring-core`)
 * accept — bundle of prompts + palette + variant modifiers.
 */
export const regionStoreConfig = {
