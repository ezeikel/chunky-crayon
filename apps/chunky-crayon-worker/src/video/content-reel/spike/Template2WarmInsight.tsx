/**
 * Template 2 — Warm Insight
 *
 * Used for fine-motor / creativity / family-bonding stats. Designed to read
 * as "this could be a moment with my kid" — warmer, slightly slower, less
 * aggressive than Template 1's Shock Stat. Same beat structure and shared
 * components; differences are the colour palette (yellow + green vs pink +
 * teal), the number anchor colour (pink vs orange), and a softer light-leak
 * tuning.
 *
...
export type Template2WarmInsightProps = {
  reel: ContentReel;
 ...
 */
export const computeWarmReelDuration = (
  hookVoiceSeconds: number = DEFAULT_HOOK_VOICE_SECONDS,
  payoffVoiceSeconds: number = DEFAULT_PAYOFF_VOICE_SECONDS,
): number =>
  computeBeats({ fps: WARM_REEL_FPS, hookVoiceSeconds, payoffVoiceSeconds })
    .totalFrames;

export const WARM_REEL_DEFAULT_DURATION_FRAMES = computeWarmReelDuration();

export const Template2WarmInsight: React.FC<Template2WarmInsightProps> = ({
  reel,
  hookVoiceUrl,
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bgCream }}>
      ...
      {/* ===== Plasma background — real GLSL shader.
          Yellow + green palette gives a "garden / morning" feel that
          contrasts with Template 1's pink + teal "evening / dramatic"
          vibe — distinct on a social-grid thumbnail at a glance. */}
      <PlasmaShader
        colorBg={COLORS.bgCream}
        colorWarm={COLORS.yellow}
        colorCool={COLORS.green}
      ...
      >
        <AbsoluteFill style={{ mixBlendMode: "screen", opacity: 0.25 }}>
          <LightLeak
            durationInFrames={Math.floor(beats.revealDur * 0.6)}
            seed={5}
            hueShift={60}
      ...
      {/* ===== Source attribution — outro fade.
          ...
          tips often don't have a citable source. */}
      {reel.sourceTitle ? (
        <div
          style={{
            position: "absolute",
      ...
      {/* ===== CTA — outro fade. Pink-dark to match this template's
          number anchor, contrasts with Template 1's orange CTA. */}
      <div
        style={{
          position: "absolute",
          bottom: 100,
      ...
      {/* SFX — Warm template uses a softer swish + chime pair on the
          stat reveal, then swaps to swoosh on logo reveal so the two
          beats don't sound identical.
          Stat reveal: swish carries the number rise, chime sparkles on
          top to land the warmth. Chime clip is 6.5s so we cap it at ~1s
          via endAt to prevent it bleeding into the payoff voice.
          Logo reveal: swoosh (punchier than the stat-reveal swish) tags
          the C-mark spring. */}
      <Sequence from={beats.revealStart}>
        <Audio src={staticFile("v2-sfx/swish.mp3")} volume={0.55} />
      </Sequence>
      <Sequence from={beats.revealStart}>
        <Audio src={staticFile("v2-sfx/chime.mp3")} volume={0.5} endAt={30} />
      </Sequence>
      <Sequence from={beats.outroStart}>
        <Audio src={staticFile("v2-sfx/textbox-swoosh.wav")} volume={0.45} />
      </Sequence>
